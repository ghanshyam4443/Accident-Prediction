...
from config import (Time_dict, Age_band_of_driver_dict, Driving_experience_dict, 
                    Service_year_of_vehicle_dict, Light_conditions_dict, 
...
try:
    ...
    st.error(f"Missing key in encoding file: {e}")
...
def main():
    with st.form('prediction_form'):
        ...
        number_of_vehicles_involved = st.slider("Pick count of vehicles involved:", 1, 7, value=1, format="%d")
        number_of_casualties = st.slider("Pick count of casualties:", 1, 8, value=1, format="%d")
    ...
    if submit:
        try:
            user_inp = np.array([
                Time_dict[time], 
                le_Day_of_week.transform([day_of_week])[0], 
                Age_band_of_driver_dict[driver_age], 
                le_Vehicle_driver_relation.transform([vehicle_relation])[0], 
                Driving_experience_dict[driving_experience], 
                Service_year_of_vehicle_dict[service_year_of_vehicle], 
                le_Road_surface_type.transform([road_surface_type])[0], 
                le_Road_surface_conditions.transform([road_surface_conditions])[0], 
                Light_conditions_dict[light_conditions], 
                le_Type_of_collision.transform([type_of_collision])[0], 
                number_of_vehicles_involved, 
                number_of_casualties, 
                le_Vehicle_movement.transform([vehicle_movement])[0], 
                Age_band_of_casualty_dict[age_band_of_casualty], 
                le_Work_of_casuality.transform([work_of_casuality])[0], 
                le_Cause_of_accident.transform([cause_of_accident])[0]
            ]).reshape(1,-1)
